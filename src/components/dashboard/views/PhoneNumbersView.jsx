...
export default function PhoneNumbersView({ onDataChange }) {
  ...
  const handleAddOrEdit = async (formData) => {
    ...
    try {
      if (editingPhone) {
        const updated = await phoneService.updatePhone(editingPhone._id, formData)
        setPhoneNumbers(phoneNumbers.map((p) => (p._id === updated._id ? updated : p)))
      } else {
        const newPhone = await phoneService.addPhone(formData)
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12 animate-in fade-in duration-500 font-sans">
      {/* Premium Hub Header */}
